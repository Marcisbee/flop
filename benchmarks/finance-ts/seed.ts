/**
 * Finance benchmark seed script
 *
 * Seeds the database with a large volume of financial data:
 *   - 1,000 users
 *   - 3,000 accounts (3 per user: checking, savings, credit)
 *   - Initial deposits into every account
 *   - 50,000 random transfers between accounts
 *
 * Usage:
 *   1. Start the server:  deno task start benchmarks/finance/app.ts
 *   2. Create admin via the setup URL printed by the server
 *   3. Run seed:  deno run --allow-net benchmarks/finance/seed.ts
 *
 * Options:
...
// Concurrency for setup steps (registration, accounts, deposits)
...
async function main() {
  ...
  await runBatched(userTemplates, SETUP_CONCURRENCY, async (u) => {
    try {
      // Register returns a token directly — no need for a separate login
      const result = await register(u.email, u.password, u.name);
      users.push({ ...u, token: result.token });
    } catch {
      // User may already exist if re-running — fall back to login
      try {

/**
 * PGlite Finance Benchmark server.
 *
...
const DB_DIR = `${import.meta.dirname}/data`;
const JWT_SECRET = "pglite-bench-secret";
...
async function getStats() {
  ...
  const byStatus = await all<{ status: string; count: number; total: number }>(
    "SELECT status, COUNT(*)::bigint AS count, COALESCE(SUM(amount), 0) AS total FROM transactions GROUP BY status",
  );
...
async function createAccount(
  ...
  const id = genId();
  await run(
    "INSERT INTO accounts (id, owner_id, name, type, balance, currency) VALUES ($1, $2, $3, $4, $5, $6)",
...
async function deposit(accountId: string, amount: number) {
  ...
  return await withTx(async () => {
    ...
    await run("UPDATE accounts SET balance = $1 WHERE id = $2", [
      newBalance,
      accountId,
    ]);
    ...
    const txId = genId();
    await run(
      "INSERT INTO transactions (id, from_account_id, to_account_id, amount, currency, status, description) VALUES ($1, $2, $3, $4, $5, $6, $7)",
      [
        txId,
        "EXTERNAL",
        accountId,
    ...
    await run(
      "INSERT INTO ledger (id, account_id, transaction_id, amount, balance_after, type) VALUES ($1, $2, $3, $4, $5, $6)",
...
async function transfer(
  ...
  return await withTx(async () => {
    const fromAccount = await one<AccountRow>(
      "SELECT id, balance, currency FROM accounts WHERE id = $1",
      [fromAccountId],
    );
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    if (pathname === "/api/auth/register" && req.method === "POST") {
      ...
      await run(
        "INSERT INTO users (id, email, password, name, roles) VALUES ($1, $2, $3, $4, $5)",
        [id, email, hash, name, "[]"],
      );
    ...
    if (pathname === "/api/auth/password" && req.method === "POST") {
      ...
      const user = await one<UserRow>("SELECT * FROM users WHERE email = $1", [
        email,
      ]);
    ...
    if (pathname === "/api/auth/refresh" && req.method === "POST") {
      ...
      const user = await one<UserRow>("SELECT * FROM users WHERE id = $1", [
        payload.sub,
      ]);
    ...
    if (pathname.startsWith("/api/view/") && req.method === "GET") {
      ...
      if (viewName === "get_accounts") {
        ...
        const rows = await all<Record<string, unknown>>(
          "SELECT * FROM accounts WHERE owner_id = $1 ORDER BY created_at DESC LIMIT 10000",
          [auth.id],
        );
        return addCors(
          jsonResponse({ ok: true, data: rows.map(snakeToCamel) }),
      ...
      if (viewName === "get_transactions") {
        ...
        const rows = await all<Record<string, unknown>>(
          "SELECT * FROM transactions WHERE from_account_id = $1 OR to_account_id = $2 ORDER BY created_at DESC LIMIT 10000",
          [accountId, accountId],
        );
        return addCors(
          jsonResponse({ ok: true, data: rows.map(snakeToCamel) }),
      ...
      if (viewName === "get_ledger") {
        ...
        const accountId = String(url.searchParams.get("accountId") ?? "");
        const rows = await all<Record<string, unknown>>(
          "SELECT * FROM ledger WHERE account_id = $1 ORDER BY created_at DESC LIMIT 10000",
          [accountId],
        );
        return addCors(
          jsonResponse({ ok: true, data: rows.map(snakeToCamel) }),
...
console.log(`\n[PGlite Benchmark] http://localhost:${PORT} (${DB_DIR})`);

...
func (p *OAuth2AuthProvider) ExchangeGrant(ctx context.Context, request AuthProviderCallbackRequest) (AuthProviderExchangeResult, error) {
	...
	identity := AuthProviderIdentity{Provider: request.Provider, Issuer: p.Definition.Issuer, Subject: subject, DisplayName: claimString(claims[p.Definition.DisplayNameClaim]), Email: claimString(claims[p.Definition.EmailClaim]), EmailVerified: claimBool(claims[p.Definition.EmailVerifiedClaim])}
...
func BuiltinOAuth2ProviderDefinition(provider string) (OAuth2ProviderDefinition, bool) {
	definitions := map[string]OAuth2ProviderDefinition{
		...
		"twitch":   {AuthorizationEndpoint: "https://id.twitch.tv/oauth2/authorize", TokenEndpoint: "https://id.twitch.tv/oauth2/token", UserInfoEndpoint: "https://api.twitch.tv/helix/users", RevocationEndpoint: "https://id.twitch.tv/oauth2/revoke", OmitClientSecretOnRevoke: true, RefreshSupported: true, Issuer: "https://id.twitch.tv/oauth2", ClientAuthStyle: AuthProviderClientSecretPost, UserInfoSubjectClaim: "id", DisplayNameClaim: "display_name", EmailClaim: "email", UserInfoHeaders: map[string]string{"Client-Id": "{client_id}"}},
		"github":   {AuthorizationEndpoint: "https://github.com/login/oauth/authorize", TokenEndpoint: "https://github.com/login/oauth/access_token", UserInfoEndpoint: "https://api.github.com/user", RevocationEndpoint: "https://api.github.com/applications/{client_id}/grant", RevocationStyle: "github_grant_delete", Issuer: "https://github.com", ClientAuthStyle: AuthProviderClientSecretPost, UserInfoSubjectClaim: "id", DisplayNameClaim: "login", EmailClaim: "email"},
		"google":   {AuthorizationEndpoint: "https://accounts.google.com/o/oauth2/v2/auth", TokenEndpoint: "https://oauth2.googleapis.com/token", UserInfoEndpoint: "https://openidconnect.googleapis.com/v1/userinfo", RevocationEndpoint: "https://oauth2.googleapis.com/revoke", RevocationTokenType: "refresh_token", RefreshSupported: true, Issuer: "https://accounts.google.com", Audience: "{client_id}", ClientAuthStyle: AuthProviderClientSecretPost, UserInfoSubjectClaim: "sub", DisplayNameClaim: "name", EmailClaim: "email", EmailVerifiedClaim: "email_verified", AuthorizationParameters: map[string]string{"access_type": "offline", "include_granted_scopes": "true"}},
		"facebook": {AuthorizationEndpoint: "https://www.facebook.com/dialog/oauth", TokenEndpoint: "https://graph.facebook.com/oauth/access_token", UserInfoEndpoint: "https://graph.facebook.com/me?fields=id,name,email", RevocationEndpoint: "https://graph.facebook.com/me/permissions", RevocationStyle: "bearer_delete", Issuer: "https://www.facebook.com", ClientAuthStyle: AuthProviderClientSecretPost, UserInfoSubjectClaim: "id", DisplayNameClaim: "name", EmailClaim: "email"},
		"x":        {AuthorizationEndpoint: "https://x.com/i/oauth2/authorize", TokenEndpoint: "https://api.x.com/2/oauth2/token", UserInfoEndpoint: "https://api.x.com/2/users/me", RevocationEndpoint: "https://api.x.com/2/oauth2/revoke", RevocationTokenType: "refresh_token", RefreshSupported: true, Issuer: "https://x.com", ClientAuthStyle: AuthProviderClientSecretBasic, UserInfoSubjectClaim: "id", DisplayNameClaim: "username"},
	}

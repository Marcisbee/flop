...
var AuthClient = class {
  ...
  async authWithOAuth2(opts) {
    ...
    return new Promise((resolve, reject) => {
      const popup = win.open(`${this.host}/api/auth/oauth2/${opts.provider}/authorize`, "flop_oauth2", `width=${width},height=${height},left=${left},top=${top}`);
      if (!popup) {
...
var Flop = class {
  ...
  watch(name, params, onData, onError) {
    ...
    this.enqueueViewCall(entry.name, entry.params, watchID).then((value) => entry.onData(value), (err) => entry.onError?.(toError(err)));
    return () => {
  ...
  async flushViewBatch() {
    ...
    try {
      ...
      const byID = new Map(results.map((r) => [
        r.id,
        r
      ]));
